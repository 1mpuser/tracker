...
export class StatsController {
  ...
  categoryStats(@CurrentUser() user: AuthUser, @Query('days') days?: string) {
    ...
    return this.statsService.categoryStats(user, Number.isNaN(parsed) ? 30 : parsed);
  }

  @Get('distraction')
  distractionWeekly(@CurrentUser() user: AuthUser, @Query('weeks') weeks?: string) {
    const parsed = weeks ? parseInt(weeks, 10) : 8;
    return this.statsService.distractionWeeklyStats(user, Number.isNaN(parsed) ? 8 : parsed);
  }

  @Get('distraction-daily')
  distractionDaily(@CurrentUser() user: AuthUser, @Query('days') days?: string) {
    const parsed = days ? parseInt(days, 10) : 30;
    return this.statsService.distractionDailyStats(user, Number.isNaN(parsed) ? 30 : parsed);
  }

  @Get('week')
  weekStats(@CurrentUser() user: AuthUser, @Query('end') end?: string) {
    // Без явного `end` «сегодня» берётся в поясе пользователя, а не UTC-сервера.
    return this.statsService.weekStats(user, end ?? formatDate(todayFor(user.timezone)));
  }
}
