...
describe('TelegramConfigService', () => {
  ...
  it('updateChat and deleteChat throw 404 for a missing chat', async () => {
    ...
    await expect(service.updateChat(5, { title: 'x' })).rejects.toThrow(NotFoundException);
    await expect(service.deleteChat(5)).rejects.toThrow(NotFoundException);
  });
  ...
  it('discover requires a bot token', async () => {
    prisma.settings.findUnique.mockResolvedValue({ id: 1, telegramBotToken: null });
    await expect(service.discover()).rejects.toThrow(ConflictException);
  });
