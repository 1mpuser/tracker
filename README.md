...
Персональный, self-hosted дневной трекер. Многопользовательский: регистрация по почте
(письмо подтверждения через Resend), вход по паролю, данные каждого изолированы.
Локально — Docker Compose; продакшен на своём VPS — см. [`docs/deploy.md`](docs/deploy.md).
...
Когда день отмечается закрытым, бэкенд публикует в Telegram-чат(ы) одно сводное
сообщение: дата с днём недели, количество помидорок, бинарный список сфер с отметками,
оценка дня и комментарий. Ровно один пост на дату на **каждый** выбранный чат —
переоткрытие и повторное закрытие дня ничего не шлёт (идемпотентность обеспечивается
таблицей `TelegramPost`, ключ `(dayId, chatId, kind)`).

**Основной способ настройки — в интерфейсе:** Настройки → «Telegram-бот» (токен
получаем у `@BotFather` через `/newbot`) и «Чаты» (куда слать сводки; чатов может быть
несколько, у каждого свои флаги «день» / «неделя», есть кнопки «Найти чаты» и «тест»).
Каждый пользователь подключает своего бота — env-фоллбэков нет.

Без настроенной интеграции она молча выключена — остальной трекер работает как обычно.
Ошибка отправки (нет сети, неверный токен, бот не админ) не ломает закрытие дня:
она пишется в лог бэкенда, а слот публикации освобождается, поэтому следующее закрытие
этого же дня попробует отправить снова.

Дни и недели, разосланные старым одноканальным кодом (поля `Day.telegramMessageId` /
`Day.weeklyTelegramMessageId`), считаются разосланными во все чаты — переоткрытие
старого дня их повторно не рассылает.
...
При закрытии **воскресенья** в выбранные чаты (с флагом «неделя») уходит вторая,
недельная сводка: картинка с графиком помидорок по дням недели и текст с итогами —
сумма и среднее за день, лучший день, средняя оценка, сферы в формате «5/7» и среднее
по «залипанию» (название настраивается в Настройки → «Залипание»; у владельца оно
выставлено в «YouTube»).

Отдельной настройки нет: работает на том же токене и тех же чатах, что дневная сводка.
Один пост на неделю на чат — переоткрытие и повторное закрытие воскресенья ничего не
шлёт (идемпотентность через `TelegramPost`).
...
`chat_id` — это просто идентификатор чата, и Telegram-у всё равно, канал это, группа
или личная переписка. Поэтому чтобы сводки дня падали в общий чат (например, вдвоём
с кем-то) — достаточно добавить этот чат вторым в Настройки → «Чаты». День уходит в чат A, потом
добавили чат B, день переоткрыли и снова закрыли → сводка уйдёт только в B.
...
**Групповой чат (через «Найти чаты»):**
...
3. В Настройки → «Чаты» нажать **«Найти чаты»** и добавить найденную группу. Так не нужно
   выковыривать `chat_id` курлом: группа появится в списке сама. Для публичного канала можно
   указать `@username` вручную, id группы отрицательный (`-123...`, супергруппа `-100...`).

**Личный чат с ботом (сводка только себе):** написать боту `/start`, затем в «Найти чаты»
добавить личный чат (у него положительный числовой id).
...
3. Заполнить в корневом `.env`:
   ...
   - `SESSION_MIN_MINUTES` — минимальная длина сеанса, который считается помидоркой
     (по умолчанию 20; отсекает перерывы и оборванные сеансы); `0` и пустое
     значение — это тоже откат к умолчанию 20, а не «считать сеансы любой длины»;
