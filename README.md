...
Переменные `TELEGRAM_BOT_TOKEN` / `TELEGRAM_CHAT_ID` в `.env` остались только как
фоллбэк для старых инсталляций: токен из `.env` работает, пока не задан свой в
настройках; env-чат используется только если таблица чатов пуста (интерфейс показывает
баннер об этом).
...
с кем-то) — достаточно добавить этот чат вторым в Настройки → «Чаты» (или, в старой
схеме, подставить его `chat_id` в `TELEGRAM_CHAT_ID`). День уходит в чат A, потом
